...
class Imagenet_dataset(object):
    ...
    def load_by_tfds(self, split_lst: List = ['train', 'validation', 'test']) -> Dict:
        ...
        ds_lst = tfds.load('imagenet2012', data_dir='/data/tf_ds',
                           download=True, as_supervised=True, split=split_lst)
        key_lst = []
        ...
        if len(split_lst) == 1:
            ...
            key_lst.append('train')
            key_lst.append('validation')
            key_lst.append('test')
        else:
            raise NotImplementedError(
                "For simplify implementation, we only support you split dataset into 3 subset at maximum : ('train', 'validation', 'test')")
        ...
        def wrap_tfds(ds):
            ...
            if FLAGS.training_loop == "two_views":
                ...
                if FLAGS.resize_wrap_ds:
                    img_lab_ds = ds.map(lambda x, y: (tf.image.resize(
                        x, img_shp), y), num_parallel_calls=AUTO)  # .cache()
    ...
    def RandAug_strategy(self, crop_type="incpt_crp", num_transform=3, magnitude=4):
        ...
        # self.strategy.experimental_distribute_dataset(train_ds)
        return train_ds
    ...
    def mixed_strategy(self, crop_type="incpt_crp", auto_policy_type="v1", Fast_policy_type="imagenet", num_transform=1, magnitude=10,):
        ...
        train_ds = ds.map(lambda x, y: ((self.SimCLR_Augment_crop(x, crop_type=crop_type), y),
                                        ...
                                        (*self.Fast_Augment_crop(x,
                                         policy_type=Fast_policy_type, crop_type=crop_type), y),
                                        (self.Rand_Augment_crop(x, num_transform, magnitude, crop_type=crop_type), y)), num_parallel_calls=AUTO) \
            .batch(self.BATCH_SIZE, num_parallel_calls=AUTO).prefetch(mode_prefetch)
    ...
    def multi_view_data_aug(self, da_func=None, da_type=None):
        ...
        train_ds = raw_ds.map(lambda x, y: mv.multi_view(
            x,  y), num_parallel_calls=AUTO)
    ...
    def multi_views_loader(self, min_scale, max_scale, crop_size, num_crops, num_transform=1, magnitude=10, policy_type="input_policy", augment_strategy="RandAug"):
        ...
        for i, num_crop in enumerate(num_crops):
            ...
            for _ in range(num_crop):
                ...
                if augment_strategy == "RandAug":
                    ...
                    trainloader = raw_ds.map(lambda x, y: (self.random_resize_crop(x, min_scale[i], max_scale[i], crop_size[i]), y), num_parallel_calls=AUTO)\
                    ...
                    img_shp = (self.IMG_SIZE, self.IMG_SIZE)
                    trainloader = raw_ds.map(lambda x, y: (self.random_resize_crop(x, min_scale[i], max_scale[i], crop_size[i]), y), num_parallel_calls=AUTO).map(
                        lambda x, y: (simclr_augment_style(x, ), y), num_parallel_calls=AUTO)

                elif augment_strategy == "FastAA":
                    print("You implement Multi-View --> FASTAA")
                    trainloader = raw_ds.map(lambda x, y: (self.random_resize_crop(x, min_scale[i], max_scale[i], crop_size[i]), y), num_parallel_calls=AUTO).map(
                        lambda x, y: (*self.Fast_Augment(x, policy_type=policy_type), y), num_parallel_calls=AUTO)
                else:
                    raise ValueError("Invalid Data Augmentation Strategies")
                # Directly apply with_option
                trainloader = trainloader.shuffle(
                    self.BATCH_SIZE*10, seed=self.seed)
        ...
        train_ds = train_ds.batch(
            self.BATCH_SIZE, num_parallel_calls=AUTO).prefetch(mode_prefetch)
