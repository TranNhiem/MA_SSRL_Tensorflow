...
##### New Section Adding 
# def rand_saturation(image: tf.Tensor) -> tf.Tensor:
#   image = tf.image.random_saturation(
#         image, lower=1-0.8*0.2, upper=1 + 0.8 * 0.2)
#   return image

# def rand_hue(image: tf.Tensor)  -> tf.Tensor:
...
class AutoAugment(ImageAugment):
  ...
  @staticmethod
  def policy_v1():  # reduced-imagenet policy
    policy = [
...
class Proposed_RandAugment(ImageAugment):
  ...
  def __init__(self,
    ...
    Args:
      ...
      translate_const: multiplier for applying translation.
    """
    super(Proposed_RandAugment, self).__init__()
    ...
    self.available_ops = [
      ...
      ## Modify Rand_Augment of Color Transformation 
        'AutoContrast', 'Equalize', 'Invert', 'Posterize', 'Solarize', 'Sharpness',
        'Color', 'Contrast', 'Brightness', 'SolarizeAdd', 
